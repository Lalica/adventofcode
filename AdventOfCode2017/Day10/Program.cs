...
namespace Day10
{
    public class Program
    {
        public static void Solution()
        {
            string s;
            string[] separators = {","};
            using (StreamReader sr = File.OpenText(Path.Combine(Directory.GetParent(Directory.GetCurrentDirectory()).Parent.FullName, "input10.txt")))
            {
                s = sr.ReadLine();
            }
            ...
            Console.WriteLine(denseHash.ToLower());
            Console.Read();
        }
    }
}
